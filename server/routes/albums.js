...
try{
  ...
  const result = await album.findByIdAndDelete(req.params.deleteId);
  if (result) {
    res.status(200).send({ success: true, msg: "album deleted successfully" });
  } else {
    res.status(404).send({ success: false, msg: "album not found" });
  }
}
catch (error) {
  res.status(500).send({ success: false, msg: "Error deleting album", error });
}
